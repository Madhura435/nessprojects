...
class CosmosCarControllerTesting {
	...
	public void getCars_200() throws Exception
	{
		...
		MvcResult mvcresult=mockMvc.perform(get(url)
				...
				.andExpect(jsonPath("$[0].name",is("TN12")))
				.andReturn();
	...
	MvcResult mvcResult=mockMvc.perform(post(url)
			...
			.contentType(MediaType.APPLICATION_JSON_VALUE))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.name",is("Dl78")))
			.andReturn();
	...
	public void getEmployeeById() throws Exception
	{
		...
		MvcResult mvcresult=mockMvc.perform(get(url)
				.contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.name",is("ke4")))
				.andReturn();
	...
	String jsonSavedCar=objectmapper.writeValueAsString(saveCar);
		...
		MvcResult updatemvcResult=mockMvc.perform(put(alterUrl)
				.content(jsonSavedCar)
				.contentType(MediaType.APPLICATION_JSON_VALUE))
				.andExpect(status().isOk())
				.andReturn();
		...
		assertEquals("Mp78",ChangedCar.getName());
	...
	public void CarNotFoundByID_partition_404 () throws Exception
	{
		...
		mockMvc.perform(get(url)
	         ...
	         .andExpect
	          ...
	          .andExpect(
	              result->assertEquals("car not found with id "+id+"in the partion"+colour,result.getResolvedException().getMessage()));
	...
	public void EmpltyInputException_404 () throws Exception
	{
		...
		mockMvc.perform(post(url)
			.content(jsonCar).contentType(MediaType.APPLICATION_JSON_VALUE))
			.andExpect
	          (result->assertTrue(result.getResolvedException() instanceof NullPointerException))
	        .andExpect(
	              result->assertEquals("input fields canNot be empty",result.getResolvedException().getMessage()));
	}
